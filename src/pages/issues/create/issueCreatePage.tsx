...
function IssueCreatePage() {
	...
	const updateProblems = (key: keyof Problem, value: any) => {
			...
			<div className="flex flex-col space-y-2 p-4 -mt-16">
				<div className="border border-gray-200 rounded-xl m-8 bg-white">
					...
					<div className="p-4 flex flex-col space-y-4">
						<TextInput
							title="Title"
							onChange={(text) => updateProblems("title", text)}
						/>
						<TextInput
							title="Description"
							onChange={(text) => updateProblems("description", text)}
						/>
						<DateInput title="Solved" placeholder="End date and time of reservation" onChange={(datetime) => updateProblems("solvedAt", datetime)}/>
						<label htmlFor="priority">Priority:</label>
						<select id="priority" name="priority" className="w-full flex flex-col" onChange={(value) => updateProblems("priority", value)}>
