...
import { CameraIcon, ChevronRightIcon, CursorClickIcon, DesktopComputerIcon, ExclamationIcon, LocationMarkerIcon } from "@heroicons/react/outline";
...
function ReservationDetailPage() {
  ...
  const onResult = (result: qrResult, error: qrError) => {
    if (result === reservationDetails?.qrToken) {
      setReservationDetails({ ...reservationDetails, scannedAt: new Date() });

      fetch(`${config.apiUrl}/reservation/${id}`, {
        method: "PUT",
        mode: "cors",
  ...
  return (
    <>
      <div>
        <h1 className=" font-SofiaProBold text-left text-black text-2xl pt-12 pl-5 ">
          Scan to sign in
        </h1>
      </div>
      ...
      <div className="block bottom-0 rounded-md fixed inset-x-0">
        <div className="rounded-md bg-white h-52 p-6 h-full drop-shadow-2xl">
          ...
          <div className="grid grid-cols-2 gap-2 py-2 place-content-between">
            ...
            <div className="text-lg text-gray ">Accessories</div>
            <div className="flex justify-end font-SofiaProBold">
