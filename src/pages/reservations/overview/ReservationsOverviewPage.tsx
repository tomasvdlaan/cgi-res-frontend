...
function ReservationsOverviewPage() {
	...
	const refresh = async () => {
		let url = `${config.apiUrl}/reservation?includePast=true`;
              ...
              <XCircleIcon className="h-4 m-3 text-gray-500" />
				...
				<div className="flex flex-col space-y-2 p-4 -mt-16">
					{reservations.map((r, index) => (
						<Link key={index} to={`/reservations/${r.id}`}>
							{r.end! < new Date() ? (
								<div
									className="rounded border border-gray-200 bg-gray-400 px-4 p-2 flex flex-row items-center space-x-4 justify-between">
									<div className="flex flex-col">
									...
									<span>
										{r.start && DateUtil.getFullDateTimeString(r.start)} -{" "}
										{r.end && DateUtil.getFullDateTimeString(r.end)}
									</span>
								</div>
							) : r.start! < new Date() ? (
								<div
									className="rounded border border-gray-200 bg-white px-4 p-2 flex flex-row items-center space-x-4 justify-between">
									<div className="flex flex-col">
                        ...
                        Now active
									...
									<span>
										{r.start && DateUtil.getFullDateTimeString(r.start)} -{" "}
										{r.end && DateUtil.getTimeString(r.end)}
									</span>
								</div>
							) : (
								<div
