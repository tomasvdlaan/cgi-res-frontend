...
function WorkspaceDetails() {
	...
	const refresh = async () => {
		let url = `${config.apiUrl}/workspace/${id}`;
	...
	return (
		<>
			<div className="flex flex-col">
				...
				<div>
					<div
						className="w-auto mx-3 my-3 border-solid border-grey-light rounded border shadow-sm bg-white -mt-16">
				...
				<div className="flex flex-col p-4 space-y-6">
					{workspace?.reservations?.length
						? workspace?.reservations.map((r, index ) => (
							<div
								key={index}
								className="border border-gray-300 rounded p-4 flex-1 flex flex-row space-x-4 items-center">
								<div className="flex-1">{r.userId}</div>
								<div>
									{r.start && DateUtil.getTimeString(r.start)} -{" "}
