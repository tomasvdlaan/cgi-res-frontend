	...
	return (
		<div
			...
			{dates.map((value, index) =>
				<div key={index} className="p-4 flex flex-col items-center snap-center rounded-md p-4 bg-white drop-shadow-md min-w-[64px]" ref={el => observer.observe(el!)} data-unix={value.unix}>
					<div>{value.day}</div>
					<div>{value.month}</div>
				</div>
			)}
